...
NoteData::NoteData(Channel *channel_, int startChunk_, AnalysisData *analysisData)
{
  ...
  _endChunk = startChunk_ + 1;
    framesPerChunk=channel_->framesPerChunk();
    NchRate= channel_->chRate() ;
  ...
  _periodOctaveEstimate = 1.0f;
  loopStep = channel->chRate() / 1000;  // stepsize = 0.001 seconds
  loopStart = _startChunk * channel_->framesPerChunk() + loopStep;
  prevExtremumTime = -1;
  prevExtremumPitch = -1;
  prevExtremum = NONE;
  maxima = new Array1d<int>();
...
{
    return (double(numChunks()) * double(framesPerChunk) / NchRate);
}
