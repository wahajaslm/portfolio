...
void AnalysisData::calcScores()
{
  ...
  {
    a[j] = bound(
                 ((*amp_mode_func[j])(values[j]) - (*amp_mode_func[j])(gdata->ampThreshold(j,0))) /
                 ((*amp_mode_func[j])(gdata->ampThreshold(j,1)) - (*amp_mode_func[j])(gdata->ampThreshold(j,0))), 0.0, 1.0
                 );
  }
    noteScore() = a[AMPLITUDE_RMS] * a[AMPLITUDE_CORRELATION];
  
  //  NSLog(@"NoteScore %f" ,noteScore());
    
    noteChangeScore() = (1.0 - a[FREQ_CHANGENESS]);
    
   // NSLog(@"NoteChangeScore %f" ,noteChangeScore());
