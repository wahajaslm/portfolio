...
Channel::Channel(BufferManager *parent_, int size_, int k_) : lookup(0, 128)/*, filteredData(0, 8192)*/
{
    
  setParent(parent_);
  int sampleRate = parent_->SampleRate();
    chSampleRate =sampleRate;
  if(k_ == 0) k_ = (size_ + 1) / 2;
  curChunk=parent->currentChunk();
  framesperchunk= parent_->framesPerChunk();
    setTimePerChunk(parent_->timePerChunk());
...
void Channel::processChunk(int chunk)
{
    ...
    if(chunk >= 0 )
    {
        curChunk=chunk;
        lookup.push_back(AnalysisData());
        parent->mFFTHelper->AudioProcessingSystem(chunk, this);
    };
...
{
  myassert(data);
  if(data->noteScore() >= gdata->ampThreshold(NOTE_SCORE,0))
  {
//      NSLog(@"Visible Chunk");
      return true;
  }
  //  NSLog(@" Not Visible Chunk");
      return false;
}
...
{
  int first = fmax(chunk - (int)ceil(longTime/timePerChunk()), getLastNote()->startChunk()/*+5*/);
 ...
 for(int curChunk=first+1, j=1; curChunk<=last; curChunk++, j++)
 {
  ...
  for(int curChunk = largestDiffChunk; curChunk < last; curChunk++)
  {
    ...
    dataAtChunk(curChunk)->shortTermDeviation = 0.1f;
...
bool Channel::isNoteChanging(int chunk)
{
    ...
    float diff = fabs(analysisData->pitch - analysisData->shortTermMean); //For short term not changing
    
    
    double spread = fabs(analysisData->shortTermMean - analysisData->longTermMean) -(analysisData->shortTermDeviation + analysisData->longTermDeviation);
 
    
    if(numChunks >= 1 && spread > 0.0)
    {
    analysisData->reason = 1;
    //backTrackNoteChange(chunk);
      //  NSLog(@"Note Change Reason 1");
        return true;
    return true;
    }
    ...
    double spread2 = fabs(analysisData->shortTermMean - firstShortData->longTermMean) -(analysisData->shortTermDeviation + firstShortData->longTermDeviation);
 
    analysisData->spread = spread;
    analysisData->spread2 = spread2;

    if(numChunks >= (int)(ceil(longTime/timePerChunk()) / 2.0) && spread2 > 0.4)
    {
    analysisData->reason = 4;
        
    //    NSLog(@"Note Change Reason 4");
    //backTrackNoteChange(chunk);
    return true;
    }

 //  NSLog(@" num chunks%d    diff %f",numChunks, diff);
    if(numChunks >= NoteNumChunks && diff >DiffParam )//*1.5*/)
    { //if jumping to fast anywhere then note is changing
       
 //       NSLog(@"Note Change Reason 2");
    analysisData->reason = 2;
    return true;
  
    }

  //  NSLog(@"Note not Changing");
return false;
}
...
void Channel::processNoteDecisions(int chunk, float periodDiff)
{
  ...
  if(noteIsPlaying)
  {
    //  NSLog(@"Note Playing Score %f" ,analysisData.noteScore());
  ...
  else
  {
      ...
      {
          //NSLog(@"Note New Score %f" ,analysisData.noteScore());
...
float Channel::calcDetailedPitch(float *input, double period, int /*chunk*/)
{
   ...
   float periodDiff = unsmoothed.back() - unsmoothed.front();
    ...
    for(j=0; j<num; j++) {
        unsmoothed[j] = freq2pitch(chRate() / unsmoothed[j]);
    }
...
void Channel::chooseCorrelationIndex1(int chunk)
{
  ...
  double freq = 44100 / analysisData.period;
...
bool Channel::chooseCorrelationIndex(int chunk, float periodOctaveEstimate)
{
  ...
  analysisData.period = analysisData.periodEstimates[choosenMaxIndex];
  //double freq = rate / period;
  double freq =  chRate()/ analysisData.period;
  analysisData.fundamentalFreq = float(freq);
  analysisData.pitch = bound(freq2pitch(freq), 0.0, gdata->topPitch());
    
  //  NSLog(@"NSDFPITCH : %f",analysisData.pitch);
  ...
  if(chunk > 0 && !isFirstChunkInNote(chunk))
  {
    ...
    //  NSLog(@"pitchSum : %f",analysisData.pitchSum);
  } else {
    analysisData.pitchSum = (double)analysisData.pitch;
