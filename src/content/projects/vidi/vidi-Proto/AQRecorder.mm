										...
										const AudioStreamPacketDescription*	inPacketDesc)
{
	...
	try {
		if (inNumPackets > 0) {
         ...
         //   if(aqr->processBufferManager->NeedsNewFFTData())
               {
        //    aqr->processBufferManager->CopyAudioDataToFFTInputBuffer((Float32*)inBuffer->mAudioData, inNumPackets);
               }
        // [[NSNotificationCenter defaultCenter] postNotificationName:@"PerformBackgroundProcessing" object:nil];
...
void AQRecorder::SetupAudioFormat(CAStreamBasicDescription	RecordFormat)
{
    mRecordFormat= RecordFormat;
}
